...
import { getSheetsWebhookUrl, fetchDatabaseFromSheets } from './lib/sheetsSync';
...
export default function App() {
  ...
  // Auto-sync on app startup: if a Google Sheets Database Webhook is configured,
  // pull the latest master data (Products, Santri, Programs, Site Config, Stats)
  // so this browser/device always shows what's currently in the Database.
  useEffect(() => {
    ...
    (async () => {
      ...
      if (res.success && res.data) {
        if (res.data.products && res.data.products.length > 0) {
          setProducts(res.data.products as SembakoProduct[]);
        }
        if (res.data.santri && res.data.santri.length > 0) {
          setSantriList(res.data.santri as Santri[]);
        }
        if (res.data.programs && res.data.programs.length > 0) {
          setPrograms(res.data.programs as ProgramTahfizh[]);
        }
        if (res.data.siteConfig) {
          setSiteConfig((prev) => ({ ...prev, ...res.data!.siteConfig }));
        }
        if (res.data.stats) {
          setStats((prev: any) => ({ ...prev, ...res.data!.stats }));
        }
      }
    })();
