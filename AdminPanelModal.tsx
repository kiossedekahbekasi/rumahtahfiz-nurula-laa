...
import { 
  ...
  syncSantriToSheets,
  syncProgramsToSheets,
  syncSiteConfigToSheets,
  syncStatsToSheets,
...
export const AdminPanelModal: React.FC<AdminPanelModalProps> = ({
  ...
  const handleSaveSiteConfig = (e: React.FormEvent) => {
    ...
    if (setSiteConfig) {
      ...
      if (isAutoSyncEnabled()) {
        syncSiteConfigToSheets(siteForm).catch(console.error);
      }

      setAuthSuccessMsg('Tulisan dan teks seluruh situs berhasil disimpan & disinkronkan!');
  ...
  const handleSaveProgram = (e: React.FormEvent) => {
    ...
    let updatedPrograms: ProgramTahfizh[];
    ...
    if (editingProgram) {
      ...
      updatedPrograms = programs.map((p) => (p.id === editingProgram.id ? updated : p));
    ...
    if (isAutoSyncEnabled()) {
      syncProgramsToSheets(updatedPrograms).catch(console.error);
    }
  ...
  // Load / Pull entire master data (Products, Santri, Programs, Site Config, Stats) FROM the Database
  const [loadDbStatus, setLoadDbStatus] = useState<{ loading: boolean; message: string; success?: boolean }>({
    loading: false,
    message: '',
  });

  const handleLoadFromDatabase = async () => {
    setLoadDbStatus({ loading: true, message: 'Menarik data terbaru dari Database Google Sheets...' });
    const res = await fetchDatabaseFromSheets();
    if (res.success && res.data) {
      if (res.data.products && res.data.products.length > 0) setProducts(res.data.products as SembakoProduct[]);
      if (res.data.santri && res.data.santri.length > 0) setSantriList(res.data.santri as Santri[]);
      if (res.data.programs && res.data.programs.length > 0 && setPrograms) setPrograms(res.data.programs as ProgramTahfizh[]);
      if (res.data.siteConfig && setSiteConfig) setSiteConfig((prev) => ({ ...prev, ...res.data!.siteConfig }));
      if (res.data.stats) setStats((prev) => ({ ...prev, ...res.data!.stats }));
    }
    setLoadDbStatus({ loading: false, message: res.message, success: res.success });
  };

  // Push ALL current master data TO the Database in one go (manual full sync)
  const [syncAllStatus, setSyncAllStatus] = useState<{ loading: boolean; message: string; success?: boolean }>({
    loading: false,
    message: '',
  });

  const handleSyncAllNow = async () => {
    setSyncAllStatus({ loading: true, message: 'Menyinkronkan seluruh data ke Database Google Sheets...' });
    const results = await Promise.all([
      syncProductsToSheets(products),
      syncSantriToSheets(santriList),
      setPrograms ? syncProgramsToSheets(programs) : Promise.resolve({ success: true, message: '' }),
      setSiteConfig && siteConfig ? syncSiteConfigToSheets(siteForm) : Promise.resolve({ success: true, message: '' }),
      syncStatsToSheets(stats),
    ]);
    ...
    setSyncAllStatus({
      ...
      success: allSuccess,
      message: allSuccess
        ? 'Semua data (Produk, Santri, Program, Statistik, Teks Situs) berhasil disinkronkan ke Database!'
        : 'Sebagian data gagal disinkronkan. Periksa Webhook URL & ukuran foto (maks. sel Sheets 50.000 karakter).',
    });
  };
  ...
  const handleSaveProduct = (e: React.FormEvent) => {
    ...
    if (isAutoSyncEnabled()) {
      syncProductsToSheets(updatedProducts).catch(console.error);
    }
  ...
  const handleSaveSantri = (e: React.FormEvent) => {
    ...
    let updatedSantri: Santri[];

    if (editingSantri) {
      updatedSantri = santriList.map((item) =>
        item.id === editingSantri.id ? ({ ...item, ...santriForm } as Santri) : item
      );
    } else {
      ...
      updatedSantri = [newSantriObj, ...santriList];
    }

    setSantriList(updatedSantri);

    if (isAutoSyncEnabled()) {
      syncSantriToSheets(updatedSantri).catch(console.error);
    }
  ...
  const handleDeleteSantri = (id: string) => {
    if (confirm('Yakin ingin menghapus data santri ini?')) {
      const updatedSantri = santriList.filter((s) => s.id !== id);
      setSantriList(updatedSantri);
                ...
                <div className="space-y-6">
                  <div className="bg-gradient-to-r from-emerald-950 via-slate-800 to-emerald-950 p-5 rounded-2xl border border-amber-400/40 space-y-2">
                    ...
                    <p className="text-xs text-slate-300">
                      Google Sheets kini menjadi <strong>database utama</strong> situs Anda. Setiap perubahan Produk, Data Santri, Program Tahfizh, Statistik, dan Teks Situs dari Panel Admin ini otomatis dikirim & tersinkron ke Spreadsheet, serta bisa ditarik ulang ke perangkat/browser lain. Pesanan sembako, transaksi donasi, dan pendaftaran santri baru juga tercatat otomatis.
                    </p>
                  </div>

                  {/* Load / Sync All Database Actions */}
                  <div className="bg-slate-800 p-5 rounded-2xl border border-amber-400/40 space-y-3">
                    <h4 className="font-bold text-sm text-white flex items-center space-x-2">
                      <RefreshCw className="w-4 h-4 text-amber-400" />
                      <span>Sinkronisasi Manual Database</span>
                    </h4>
                    <p className="text-xs text-slate-300">
                      Gunakan tombol ini untuk menarik data terbaru dari Sheets (misalnya setelah mengedit dari perangkat lain), atau mendorong semua data yang ada saat ini ke Database sekaligus.
                    </p>
                    ...
                    <div className="flex flex-wrap gap-3">
                      <button
                        ...
                        className="px-4 py-2.5 bg-emerald-800 hover:bg-emerald-700 text-emerald-100 font-bold text-xs rounded-xl border border-emerald-700 transition-colors flex items-center space-x-1.5 disabled:opacity-60"
                      >
                        <RefreshCw className={`w-3.5 h-3.5 text-amber-300 ${loadDbStatus.loading ? 'animate-spin' : ''}`} />
                        <span>Muat / Tarik Data dari Database</span>
                      </button>
                      ...
                      <button
                        ...
                        className="px-4 py-2.5 bg-amber-400 hover:bg-amber-300 text-emerald-950 font-bold text-xs rounded-xl shadow transition-colors flex items-center space-x-1.5 disabled:opacity-60"
                      >
                        <Send className={`w-3.5 h-3.5 ${syncAllStatus.loading ? 'animate-pulse' : ''}`} />
                        <span>Sinkronkan Semua Data Sekarang</span>
                      </button>
                    </div>

                    {loadDbStatus.message && (
                      <div className={`p-3 rounded-xl text-xs font-semibold ${
                        loadDbStatus.success ? 'bg-emerald-950 text-emerald-200 border border-emerald-700' : 'bg-slate-900 text-amber-300 border border-slate-700'
                      }`}>
                        {loadDbStatus.message}
                      </div>
                    )}
                    ...
                    {syncAllStatus.message && (
                      <div className={`p-3 rounded-xl text-xs font-semibold ${
                        syncAllStatus.success ? 'bg-emerald-950 text-emerald-200 border border-emerald-700' : 'bg-slate-900 text-amber-300 border border-slate-700'
                      }`}>
                        {syncAllStatus.message}
                      </div>
                    )}
                    ...
                    <p className="text-[11px] text-slate-400">
                      Catatan: 1 sel Google Sheets maksimal 50.000 karakter. Jika sinkronisasi foto gagal, gunakan URL gambar (link) daripada mengunggah file besar langsung.
                    </p>
                  </div>
                  ...
                  <form onSubmit={handleSaveWebhook} className="bg-slate-800 p-5 rounded-2xl border border-slate-700 space-y-4">
                    ...
                    <div className="flex items-center space-x-2 pt-1">
                      ...
                      <label htmlFor="autoSyncCheck" className="text-xs text-slate-200 cursor-pointer font-semibold">
                        Aktifkan Otomatis Sinkronisasi Realtime (Checkout, Pendaftaran, & setiap Simpan di Panel Admin)
                      </label>
                    </div>
                ...
                <div className="space-y-6">
                  <div className="bg-slate-800 p-5 rounded-2xl border border-slate-700 space-y-4">
                    ...
                    <button
                      ...
                      onClick={() => {
                        if (isAutoSyncEnabled()) {
                          syncStatsToSheets(stats).catch(console.error);
                        }
                        setAuthSuccessMsg('Statistik berhasil disimpan & disinkronkan ke Database!');
                        setTimeout(() => setAuthSuccessMsg(''), 4000);
                      }}
                      className="px-5 py-2.5 bg-amber-400 hover:bg-amber-300 text-emerald-950 font-bold text-xs rounded-xl shadow transition-colors flex items-center space-x-1.5"
                    >
                      <RefreshCw className="w-3.5 h-3.5" />
                      <span>Simpan & Sinkronkan Statistik ke Database</span>
                    </button>
